...
#include "../include/sirgvariants.h"
...
__global__ void firstFit(int* d_numVertices, const int* __restrict__ d_nonZeroIndices, const int* __restrict__ d_numColIndices, int* d_globalColoring, int* maxColor, long long int* adjColors, int* incSize){
  ...
  for (int id = threadID; VerticesPerThread > 0 && id < *d_numVertices; id += totalThreads, VerticesPerThread--){
      ...
      for (int i = start; i < end; i++){
        ...
        int adjColorIndex = id * maxColorVal + color/64;
        if (color == *maxColor + 1)
          continue;
      ...
      for (i = 0; i < maxColorVal; i++){
        int adjColorIndex = id * maxColorVal + i;
...
int* colorGraph(int numVertices, int* nonZeroIndices, int* numColIndices, int numElems, int* nextVertexIndices, int maxDeg){
  ...
  for(int i = 0; i < ITERATIONS; i++){
    ...
    int blocks = maximum_residency(conflictResolve, BLOCKSIZE, 0) * numSMs;
    blocks = numSMs;
    int* inlist, *outlist;
    ...
    CUDA_SAFE_CALL(cudaMalloc((void **)&d_adjColors, sizeof(long long int)*numVertices*(ceil(maxColor/64.0))));
    cudaMemset((void *)d_adjColors, 0xffffffff, sizeof(long long int)*numVertices*(ceil(maxColor/64.0)));
...
//  cout << endl;
  ...
  cout << fixed;
  cout << averageKernelRuntime << " " << averageColors << " " << sizeof(int)*numVertices*(ceil(maxColor/64.0)) << endl;
/*
  cout << "Average Runtime: " << averageKernelRuntime << endl;
  cout << "Average Colors: " << averageColors << endl;
  cout << "Memory Used for adjColors array: " << sizeof(int)*numVertices*(ceil(maxColor/64.0)) << " bytes" << endl;
*/
  return globalColors;
}

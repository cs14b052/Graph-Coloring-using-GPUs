...
int numColors(int m, int* coloring){
  set<int> hash;
  for (int i = 0; i < m; i++){
    hash.insert(coloring[i]);
  }
  return hash.size();
}
...
int main(int argc, char *argv[]) {
  ...
  size_t found = filename.find_last_of("/\\");
    ...
    cout << filename.substr(found+1, filename.length()-6);
		graph2csr(argv[1], m, nnz, csrRowPtr, csrColInd);
  }
	else if (strstr(argv[1], ".gr")){
    cout << filename.substr(found+1, filename.length()-3);
...
//  cout << "Num Colors: " <<  numColors(m, coloring) << endl;
/*
	if (correct)
		printf("correct.\n");
	else
